...
const Navbar = ({ notifications, clearNotifications }) => {
  ...
  return (
    <nav className="navbar navbar-expand-lg navbar-light bg-light shadow-sm" style={{ height: "120px" }}>
      <div className="container-fluid d-flex align-items-center" style={{ height: "100%" }}>
        <Link href="/" className="navbar-brand d-flex align-items-center">
          <Image src="/assets/images/logo.png" alt="DataSense Logo" width={100} height={100} className="logo" />
        </Link>

        <button className="navbar-toggler" type="button" onClick={() => setIsOpen(!isOpen)} aria-expanded={isOpen ? "true" : "false"} aria-label="Toggle navigation">
          <span className="navbar-toggler-icon"></span>
        </button>

        <div className={`offcanvas offcanvas-start ${isOpen ? "show" : ""}`} aria-labelledby="offcanvasNavbarLabel">
          <div className="offcanvas-header">
            <h5 className="offcanvas-title" id="offcanvasNavbarLabel">DataSense</h5>
                ...
                <>
                  <li className="nav-item">
                    <Link href="/dashboard" className="nav-link link-hover">
                      <i className="fa-sharp fa-solid fa-table-columns me-1"></i> Dashboard
                    </Link>
                  </li>
                  <li className="nav-item">
                  ...
                  <>
                    ...
                    <li className="nav-item">
                      <Link href="/terms" className="nav-link link-hover">
                        <i className="fa fa-file-contract me-1"></i> Terms & Conditions
                      </Link>
                    </li>
                  </>
                )
        ...
        <div className="ms-auto d-flex align-items-center">
          ...
          {showNotifications && (
            <div className="notification-dropdown">
              <h6 className="d-flex justify-content-between align-items-center">
                Notifications
                <button className="btn btn-sm btn-danger" onClick={clearNotifications}>
                  Clear All
                </button>
              </h6>
          ...
          {user && (
            <FiUser size={24} className="me-2" onClick={goToAccountSettings} style={{ cursor: "pointer" }} />
          )}
